...
export default function SignUp({ onBackClick, onSMSSent }) {
  ...
  function sendActivationSMS() {
    ...
    api.auth
      .register({ phone_number: phone, risk_group: condition })
      .then(res => {
        setIsDialogOpen(false);
        onSMSSent({
          phone,
          condition,
          ttl: res.activate_ttl
        });
      })
      .catch(err => {
        console.error(err);
      ...
      <div className={styles.container}>
        ...
        <Box mt={2}>
          <FormControl fullWidth>
            ...
            >
              <MenuItem value={0}>بدون شرایط خاص</MenuItem>
              <MenuItem value={1} className={styles.conditionItems}>
                بیماران با نقص ایمنی: تحت درمان با کورتیکواستروئید، شیمی درمانی،
                بدخیمی‌ها، پیوند اعضا، مبتلایان به HIV
              </MenuItem>
